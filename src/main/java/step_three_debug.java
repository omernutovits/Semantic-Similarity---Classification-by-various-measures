...
public class step_three_debug {
    ...
    public static class MapperClass_count_of_l_and_f extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable lineId, Text line, Context context) throws IOException,  InterruptedException {
            String [] lexema_count = line.toString().split("\\s+");
            context.write(new Text(lexema_count[0] + " yyyyyy " + lexema_count[1] ), new Text(lexema_count[2]));
        }
    }
            ...
            if(key_as_string[1].equals("xxxxxx")){
                if(count_of_l != null){
                    for(int i=0; i<vector.length; i++){
                        val.append(Double.toString(vector[i]));
                        val.append(" ");
                    }

                    context.write(new Text(key_as_string[0]), new Text(val.toString()));
                    vector = new Double[4000];
                    Arrays.fill(vector, 0.0);
//                    Arrays.fill(vector,	Double.NEGATIVE_INFINITY);
                }
...
//                    eq eight = ( P(l,f) - (P(l) * P(f)) ) / sqrt ( (P(l) * P(f) )
        ...
        protected void cleanup(Reducer<Text, Text, Text, Text>.Context context) throws IOException, InterruptedException {
            ...
            context.write(new Text(last_round), new Text(val.toString()));
